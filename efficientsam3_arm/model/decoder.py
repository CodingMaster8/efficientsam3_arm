...
from efficientsam3_arm.sam.transformer import RoPEAttention
...
from .box_ops_arm import box_cxcywh_to_xyxy
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn(self, tgt):
            # Dynamically get the device type from the input tensor
            # This will be 'cuda', 'cpu', or 'mps' (for Apple Silicon)
            device_type = tgt.device.type
            
            # 'mps' does not always support autocast depending on the PyTorch version.
            # Since 'enabled=False' is set here, we just need a valid string to prevent crashing.
            # If the device is 'mps', we can safely pass 'cpu' or just keep 'mps' if on newer PyTorch.
            # The safest robust fallback for disabling autocast is just using the device type directly.
